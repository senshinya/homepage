...
/**
 * 取任意外链的 Open Graph 元数据，供碎语的链接预览卡使用（见 app/components/partial/MemoLink.vue）。
 *
 * 为什么要有这个端点：碎语是运行时数据，构建期无从枚举里头的链接；而浏览器直接去
 * fetch 别人的站必被 CORS 拦下——对方没有理由给我们回 Access-Control-Allow-Origin。
 * 于是把请求收回同源，由服务端代取。
 *
 * 这是全站唯一的运行时函数，其余页面仍是构建期预渲染的静态产物。**因此构建命令
 * 必须是 `nuxt build` 而不是 `nuxt generate`**——generate 会强制 static preset
 * 把服务端整个丢掉，这个端点便不存在（线上 404，卡片退回打底态）。
 * 相应地 nuxt.config 里要显式开 prerender.crawlLinks，否则页面会从静态退化成
 * 按请求 SSR。见 README「部署」。
 *
 * 实现移植自博客同名端点。刻意各留一份而不是跨站调用博客那个：跨站要博客侧维护
 * 一张主页 Origin 的白名单，主页每换一个域名或端口（线上、localhost、局域网 IP）
 * 都要去改另一个仓库；自己带一份则同源直连，没有 CORS，也没有跨仓库的耦合。
 */
 ...
 */
const USER_AGENT = `Mozilla/5.0 (compatible; ShinyaLinkPreview/1.0; +${homepageConfig.url})`
...
function decodeHtml(bytes: Uint8Array) {
	...
	const charset = probe.match(CHARSET_RE)?.[1]
...
function parseMetaTags(html: string) {
	const head = html.split(HEAD_END_RE)[0] ?? html
