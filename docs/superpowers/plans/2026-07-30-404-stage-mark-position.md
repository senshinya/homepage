...
**Architecture:** Preserve the existing Grid layout for the error-page content, but remove `.stage-mark` from that Grid track's sizing. Make `.stage` the containing block and position the decorative mark against a stable center anchor; no JavaScript state or viewport-fixed positioning is introduced.
...
```js
test('404 backdrop stays anchored while error content changes height', () => {
	...
	assert.match(markStyles, /inset-block-start:\s*50%/)
	assert.match(markStyles, /inset-inline-start:\s*50%/)
	assert.match(markStyles, /transform:\s*translate\(-50%, -100%\)/)
